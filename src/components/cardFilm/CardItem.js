...
function CardItem({film, type = 'movie'}) {
    const [updateFavList, setUpdateList] = useState([])
    const dispatch = useDispatch()
    const { userID, favFilms } = useSelector(state => state.user)
    ...
    const handleFavourite = async () => {
        try {
            ...
            setUpdateList([...updateFavList, cardData])

            // if (favFilms.length !== 0) {
            //     favFilms.filter(item => item.id !== cardData.id)
            // } else {
            //     console.log(newArr)
            //     return newArr.push(cardData)
            // }

            // dispatch(setFavFilms(newArr))
  ...
  return (
    <div className='card'>
        ...
        <svg onClick={() => handleFavourite()} className='fav' version="1.1"  x="0px" y="0px" viewBox="0 0 471.701 471.701">
